...
def heatmap_contingency(data, title, yticklabels, xticklabels):
    ...
    g = sns.heatmap(data, cmap='bwr_r', linecolor='white', linewidth=5, center=0,
                    vmax=2, vmin=-4, square=False, cbar=True,
                    ax=ax,
    ...
    # Osh2 colorscale
    cbar.set_ticks([-4.0, -2.0, 0, 2.0])
    cbar.set_ticklabels(['-4', '-2', '0', '2'], fontsize=16, weight='bold')
    ...
    plt.savefig(figure_dir + '6C.pdf', dpi=600, bbox_inches='tight')
    plt.show()
