...
for prey in preys:
    ...
    df53 = pd.read_csv(input_dir + prey + '_MYO53.dat', sep='\t')

    df3['mutated_reference'] = df3['position'].astype(str) + df3['mut_aa']
    df53['mutated_reference'] = df53['position'].astype(str) + df53['mut_aa']

    df3 = df3[['sequence_type', 'mutated_reference', 'dF', 'classification']].copy()
    df53 = df53[['sequence_type', 'mutated_reference', 'dF', 'classification']].copy()

    # remove synonymous mutations
    df3 = df3[df3['sequence_type'] != 'synonymous'].copy()
    df53 = df53[df53['sequence_type'] != 'synonymous'].copy()
    ...
    df53['dF'] = np.clip(df53['dF'].values, a_min=0.0, a_max=1.0)

    df = pd.merge(df3, df53, on='mutated_reference')
...
ax.set_title('${{\Delta}F_{MYO5}^{Myo3}} - {{\Delta}F_{MYO3}^{Myo3}}$', weight='bold', fontsize=24)
plt.show()
