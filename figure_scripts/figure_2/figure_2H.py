...
for prey in preys:
    ...
    df35 = pd.read_csv(input_dir + prey + '_MYO35.dat', sep='\t')
    ...
    df35 = df35[['sequence_type', 'mutated_reference', 'dF', 'classification']].copy()

    # remove synonymous mutations
    df3 = df3[df3['sequence_type'] != 'synonymous'].copy()
    df35 = df35[df35['sequence_type'] != 'synonymous'].copy()
    ...
    df35['dF'] = np.clip(df35['dF'].values, a_min=0.0, a_max=1.0)

    df = pd.merge(df3, df35, on='mutated_reference')
...
ax.set_title('${{\Delta}F_{MYO3}^{Myo5}} - {{\Delta}F_{MYO3}^{Myo3}}$', weight='bold', fontsize=24)
plt.show()